    ...
    >
      {solutions.map((solution, index) => (
        <motion.div key={index} variants={item}>
          <Card variant="glass" className="h-full hover:border-pink-600/40 transition-all duration-200 group">
            <CardHeader>
              <div className="w-12 h-12 rounded-lg bg-pink-600/10 flex items-center justify-center mb-4 group-hover:bg-pink-600/20 transition-colors">
                <solution.icon className="w-6 h-6 text-pink-400" />
              </div>
              ...
              <ul className="space-y-2">
                {solution.features.map((feature, i) => (
                  <li key={i} className="text-sm text-gray-400 flex items-start">
                    <span className="w-1.5 h-1.5 bg-pink-600 rounded-full mr-2 mt-1.5 flex-shrink-0" />
                    <span>{feature}</span>
                  </li>
                ))}
              </ul>
