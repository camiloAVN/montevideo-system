...
import { useCateringMenuItemsStore } from '@/store/cateringMenuItemsStore'
...
export function useCateringMenuItems() {
  const {
    ...
    setLoading,
    setError,
  } = useCateringMenuItemsStore()

  const fetchMenus = useCallback(async () => {
    setLoading(true)
    setError(null)
    try {
      const response = await fetch('/api/catering/menus/from-items')
      if (response.status === 401 || response.status === 403) { setMenus([]); return }
      if (!response.ok) throw new Error('Error al obtener menús por items')
  ...
  const fetchMenu = useCallback(async (id: string) => {
    ...
    setError(null)
    try {
      const response = await fetch(`/api/catering/menus/from-items/${id}`)
  ...
  const createMenu = useCallback(async (data: CateringMenuFromItemsFormData) => {
    setLoading(true)
    setError(null)
    try {
      const response = await fetch('/api/catering/menus/from-items', {
  ...
  const editMenu = useCallback(async (id: string, data: CateringMenuFromItemsFormData) => {
    setLoading(true)
    setError(null)
    try {
      const response = await fetch(`/api/catering/menus/from-items/${id}`, {
  ...
  const deleteMenu = useCallback(async (id: string) => {
    ...
    setError(null)
    try {
      const response = await fetch(`/api/catering/menus/from-items/${id}`, { method: 'DELETE' })
      if (!response.ok) {
        const error = await response.json()
