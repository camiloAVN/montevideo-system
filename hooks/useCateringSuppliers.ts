...
import { useCateringSupplierStore } from '@/store/cateringSupplierStore'
...
export function useCateringSuppliers() {
  const {
    ...
    setError,
    setSearchQuery,
  } = useCateringSupplierStore()

  const fetchSuppliers = useCallback(async (search?: string) => {
    setLoading(true)
    setError(null)
    try {
      const url = new URL('/api/catering/suppliers', window.location.origin)
  ...
  const fetchSupplier = useCallback(async (id: string) => {
    ...
    setError(null)
    try {
      const response = await fetch(`/api/catering/suppliers/${id}`)
  ...
  const createSupplier = useCallback(async (data: CateringSupplierFormData) => {
    setLoading(true)
    setError(null)
    try {
      const response = await fetch('/api/catering/suppliers', {
        method: 'POST',
  ...
  const editSupplier = useCallback(async (id: string, data: CateringSupplierFormData) => {
    setLoading(true)
    setError(null)
    try {
      const response = await fetch(`/api/catering/suppliers/${id}`, {
